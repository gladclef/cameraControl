...
class ProjectInstaller {
	...
	public function check_ini_files() {
		...
		global $global_opened_db;
		$server_file = dirname(__FILE__)."/server_config.ini";
		$mysql_file = dirname(__FILE__)."/mysql_config.ini";
	...
	public function check_installed() {
		...
		if ($this->check_install_database())
		{
			...
			{
				echo "Can't init database!<br />";
				$retval = FALSE;
			}
		}
		else
		{
			echo "Can't install database!<br />";
			$retval = FALSE;
		}
		if (!$this->check_install_database())
		{
		...
		if (!$this->check_ini_files())
		{
			echo "Can't find .ini files!<br />";
			$retval = FALSE;
		}
		if (!$this->check_jquery_path_validity())
		{
			echo "Can't find JQuery!<br />";
			$retval = FALSE;
		}
		return $retval;
	}
